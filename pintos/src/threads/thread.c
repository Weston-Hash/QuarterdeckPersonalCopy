...
void thread_exit (void)
{
  ...
  process_exit ();
#endif
  /*  Marking thread as dying but not killing it yet */
  intr_disable ();
  ...
  if (prev != NULL && prev->status == THREAD_DYING && prev != initial_thread)
    {
      ASSERT (prev != cur);
#ifdef USERPROG
      /* Child already removed itself from all_list in process_exit (). */
